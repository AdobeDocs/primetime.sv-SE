...
# VMAP-format för spårning av URL:er {#vmap-format-for-tracking-urls}
...
## Ett VMAP-format {#vmap}

Den VMAP-fil som manifestservern skickar om `pttrackingversion=vmap` har formatet i följande exempel, som kommer från ett typiskt VMAP-block. Den har förkortats för att undvika onödig upprepning, så att strukturen blir tydligare. En ellips (tre punkter, avgränsade med blanksteg) anger utelämnad information i vissa URL-adresser och mellan vissa kodblock. Oförkortade URL:er visas på flera rader, men de visas på en rad i VMAP-filen.
