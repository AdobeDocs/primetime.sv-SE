...
2. Klientlöst API - RESTful web services for devices that cannot render web pages (t.ex. set-top boxes, game consoles, smart TVs). Kravet på att återge webbsidor bygger på MVPD:s krav på att användarna ska autentisera sig på MVPD:s webbplats.
...
* `<FQDN>/.../config` - Returnerar listan över MVPD-filer till andra skärmprogrammet.
...
* Det MVPD som är associerat med AuthN-token finns i listan över tillåtna MVPD-filer för det aktuella begärande-ID:t
...
* `getAuthorization()` - Initierar hela auktoriseringsflödet.
...
* `\<FQDN\>/.../authorize` - Initierar hela auktoriseringsflödet.
...
1. Tillhandahåll återanropsfunktioner för att hantera resultatet av auktoriseringen:
   ...
   * `setToken()` - Om auktoriseringen lyckas eller om användaren tidigare har auktoriserats fortsätter åtkomstaktiveringen med auktoriseringsprocessen genom att ringa `setToken()` callback-funktionen, skickar den kortlivade auktoriseringstoken.
